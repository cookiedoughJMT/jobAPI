...
4. **부족한 영역 분석**: 다음 4가지 핵심 영역을 **엄격한 기준**으로 평가하여 부족한 영역 도출
   
   ⚠️ **엄격한 평가 기준**: 
   - **단순 언급 ≠ 충분한 설명**: 해당 영역이 언급되어도 구체성과 깊이가 부족하면 부족한 영역으로 분류
   - **높은 완성도 요구**: 각 영역에서 구체적 사례 + 과정 + 결과가 모두 포함되어야 "충분히 다뤄짐"으로 판단
   - **대부분의 자소서는 개선 여지 존재**: 완벽한 자소서는 매우 드물며, 대부분 2-3개 영역에서 보완이 필요함
   
   ✅ **직무 역량 표현** (엄격 기준)
   - **문제해결 역량**: 구체적 문제 상황 → 해결 과정 → 결과까지 완전한 스토리
   - **협업 역량**: 단순 "팀워크" 언급이 아닌, 역할 분담, 갈등 조정, 소통 방식 등 구체적 사례
   - **기술 역량**: 기술/도구 나열이 아닌, 실제 적용 과정과 성과가 포함된 경험
   - **전문성**: 자격증/수상 나열이 아닌, 취득 과정의 어려움과 극복 스토리
   
   ✅ **성실성과 태도** (엄격 기준)
   - **지속성 증명**: 단순 "꾸준히" 표현이 아닌, 구체적 기간과 지속 근거 제시
   - **자기계발 노력**: 학습 결과가 아닌, 학습 동기와 과정, 적용 사례까지 포함
   - **책임감**: "끝까지 완수" 표현이 아닌, 어려움 극복과 결과 달성 구체적 사례
   - **성장 의지**: 추상적 다짐이 아닌, 구체적 계획과 실행 의지 표현
   
   ✅ **리더십과 도전정신** (엄격 기준)
   - **리더십 발휘**: 단순 "리더 역할" 언급이 아닌, 구성원 동기부여, 갈등 해결 등 구체적 리더십 행동
   - **도전과 실패**: 성공 사례만이 아닌, 실패 경험과 극복 과정, 배운 점까지 포함
   - **혁신 시도**: "새로운 아이디어" 언급이 아닌, 구체적 개선사항과 실행 결과
   - **변화 주도**: 수동적 참여가 아닌, 능동적으로 변화를 이끈 경험
   
   ✅ **결과 및 성과 중심 표현** (엄격 기준)
   - **정량적 성과**: 모호한 "많이", "크게" 표현이 아닌, 구체적 수치와 지표 제시
   - **영향력 증명**: 개인 성과가 아닌, 팀/조직에 미친 긍정적 영향 구체적 서술
   - **피드백 수용**: 단순 "개선했다" 표현이 아닌, 비판 수용 과정과 변화 결과 상세 설명
   - **성찰과 학습**: 경험 나열이 아닌, 각 경험에서 얻은 교훈과 향후 적용 방안
...
4. 각 문장을 체계적으로 개선 (개선사항이 있으면 반드시 corrections에 추가)
   ...
   - 문체를 자연스러운 문어체로 통일
   - 앞뒤 문맥을 고려한 연결성 확보
5. **엄격한 부족 영역 판단**: 전체 자소서를 위 4가지 핵심 영역으로 **높은 기준**으로 분석
   - **엄격한 완성도 평가**: 각 영역에서 구체적 사례 + 상세한 과정 + 명확한 결과가 모두 포함되어야 "충분"으로 판단
   - **대부분 자소서는 개선 필요**: 4개 영역 모두 완벽한 자소서는 극히 드물며, 보통 2-3개 영역에서 보완점 존재
   - **missing_areas = [] 조건**: 4개 영역 모두에서 높은 수준의 구체성과 완성도를 갖춘 예외적인 경우에만
...
{{
  "analysis": {{
    ...
    ⚠️ **엄격한 평가 원칙**: 4개 영역 모두 높은 완성도를 갖춘 매우 드문 경우에만 빈 배열 반환
  }}
}}
