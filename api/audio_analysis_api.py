...
# 임시 파일 처리만 사용하므로 로컬 저장 디렉토리는 불필요
...
async def analyze_audio(file: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            # WebM → WAV 변환 (임시 파일 사용)
            convert_webm_to_wav(temp_webm_path, temp_wav_path)

            # 1) Whisper ---------------------------------------------------
            whisper_result = whisper_model.transcribe(
                temp_wav_path, word_timestamps=True, language="ko"
            )
            ...
            # 2) pyAudioAnalysis ---------------------------------------------------
            Fs, x = audioBasicIO.read_audio_file(temp_wav_path)
            ...
            # 파일 사이즈 계산 (원본 업로드 데이터 기준)
            file_size = len(raw)

            # WAV 파일을 바이너리로 읽어서 base64로 인코딩
            wav_binary_base64 = None
            with open(temp_wav_path, "rb") as f:
                wav_binary = f.read()
                wav_binary_base64 = base64.b64encode(wav_binary).decode('utf-8')
            ...
            return {
                ...
                "gpt_feedback": parsed,
                "webmFn": str(webm_fn),
                "webmPath": "",  # 로컬 저장하지 않으므로 빈 문자열
                "wavPath": "",   # 로컬 저장하지 않으므로 빈 문자열
                "wavBinaryBase64": wav_binary_base64,  # base64 인코딩된 바이너리 데이터
